...
- **`app/src/app/features/paciente/home/paciente-home.page.ts`**:
  ...
  - `loadDashboard` — al recibir `citaPendienteConfirmacion` con valor, asigna los datos y activa el modal.
  - Nuevo método `onConfirmarAsistencia()` — llama al servicio, maneja loading, cierra el modal al éxito y muestra toast de confirmación.
  - Nuevo método `onCancelarDesdeModal()` — redirige al flujo de cancelación existente usando `cita-detalle`.
